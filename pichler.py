...
class Pichler:
    ...
    def setpoint_write_value(self, address, obj, value):
        """
        Write raw value to single setpoint
        ...
        value : int
        """
        return self.setpoint_write_list_values([(address, obj, value)])

    def setpoint_write_list_values(self, lst):
        """
        Write raw values to multiple setpoints.

        Parameters
        ----------
        lst : list of (address, obj, value) to write.
        """
        l = [{'address': addr, 'obj': obj, 'value': val} for addr, obj, val in lst]
        request = {'request': {'list': l}}
        print(request)
        return self.rpc_invoke('setpointWriteValue', 'json=%s' % json.dumps(request))
